...
class ConfigClient(ABC):
    ...
    @abstractmethod
    def read_fan_config(self, profile_id: int, fan_id: int) -> Optional[Dict[str, Any]]:
        """Read fan configuration."""
        pass
    
    @abstractmethod
    def write_fan_config(self, profile_id: int, fan_id: int, config: Dict[str, Any]) -> bool:
    ...
    @abstractmethod
    def write_rgb_config(self, profile_id: int, config: Dict[str, Any]) -> bool:
...
class DirectConfigClient(ConfigClient):
    ...
    def __init__(self, port: str):
        ...
        logger.info(f"Connected to device on {port}, Product ID: 0x{self.product_id:02X}")
    ...
    def read_fan_config(self, profile_id: int, fan_id: int) -> Optional[Dict[str, Any]]:
        ...
        config = read_fan_profile(self.ser, fan_profile=profile_id, fan_id=fan_id)
        if not config:
            return None
    ...
    def write_fan_config(self, profile_id: int, fan_id: int, config: Dict[str, Any]) -> bool:
        ...
        return write_fan_profile(self.ser, fan_profile=profile_id, fan_id=fan_id, config=cfg)
        ...
        if isinstance(data, dict):
            ...
            for field_name, field_type in struct_type._fields_:
                if field_name in data:
                    ...
                    if hasattr(field_type, '_type_') and hasattr(field_type, '_length_'):
                        ...
                        if isinstance(value, list):
                            ...
                            if hasattr(elem_type, '_fields_'):
                                # Array of structures
                                converted = [self._dict_to_struct(item, elem_type) for item in value]
                                setattr(struct_obj, field_name, array_type(*converted))
                            else:
                                # Array of primitives
                                setattr(struct_obj, field_name, array_type(*value))
                        else:
                            setattr(struct_obj, field_name, value)
                    elif hasattr(field_type, '_fields_'):
                        # It's a nested structure
                        setattr(struct_obj, field_name, self._dict_to_struct(value, field_type))
                    else:
                        # Primitive type
                        setattr(struct_obj, field_name, value)
            return struct_obj
...
class NamedPipeConfigClient(ConfigClient):
    ...
    def __init__(self, pipe_name: str):
        ...
        if not sys.platform.startswith("win"):
            raise RuntimeError("NamedPipeConfigClient is only supported on Windows")
    ...
    def _open_pipe(self):
        ...
        try:
            ...
            raise ConnectionError(f"Pipe '{self.pipe_name}' not available: {e}") from e
    ...
    def _send_command(self, command: str, payload=None) -> Optional[Dict[str, Any]]:
    ...
    def write_fan_config(self, profile_id: int, fan_id: int, config: Dict[str, Any]) -> bool:
        """Write fan configuration."""
        result = self._send_command(f"WriteFanConfig:{profile_id}:{fan_id}", payload=config)
        return result and result.get('success', False)
    ...
    def write_rgb_config(self, profile_id: int, config: Dict[str, Any]) -> bool:
        """Write RGB configuration."""
        result = self._send_command(f"WriteRgbConfig:{profile_id}", payload=config)
        return result and result.get('success', False)
