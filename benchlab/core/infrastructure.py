...
class InfrastructureManager:
    ...
    def start_fastapi(self, wait_ready: bool = True, timeout: float = 10.0) -> bool:
        ...
        with self._lock:
            ...
            if self._is_port_in_use(self.fastapi_host, self.fastapi_port):
                logger.info(f"Port {self.fastapi_port} already in use, assuming FastAPI is running")
                return True
            ...
            try:
                ...
                if wait_ready:
                    if self._wait_for_port(self.fastapi_host, self.fastapi_port, timeout):
                        logger.info(f"FastAPI server started on port {self.fastapi_port}")
                        return True
                    else:
    ...
    def start_mqtt_publisher(self) -> bool:
        ...
        with self._lock:
            ...
            logger.info(f"Starting MQTT publisher to {self.mqtt_broker}:{self.mqtt_port}")
