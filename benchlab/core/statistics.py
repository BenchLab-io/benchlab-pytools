...
class ChannelStats:
    ...
    def __init__(self):
        ...
        self._data: Dict[str, Dict[str, Dict[str, float]]] = defaultdict(lambda: defaultdict(_make_stat))
    ...
    def get(self, device: str, channel: str) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    def get_all(self, device: str) -> Dict[str, Tuple[Optional[float], Optional[float], Optional[float]]]:
        ...
        with self._lock:
            ...
            for channel in self._data[device]:
                ...
                if s['count'] == 0:
                    ...
                    result[channel] = (s['min'], s['max'], s['sum'] / s['count'])
            return result
...
class StatsFormatter:
    ...
    @staticmethod 
    def format_compact_range(min_val: Optional[float], max_val: Optional[float],
                           avg_val: Optional[float], decimals: int = 1, 
                           unit: str = "") -> str:
        ...
        return f"{min_val:.{decimals}f}-{max_val:.{decimals}f} ~{avg_val:.{decimals}f}{unit}"
