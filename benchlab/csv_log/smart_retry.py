...
class AdaptiveRetryManager:
    ...
    def _calculate_adaptive_delay(self, attempt: int) -> float:
        ...
        with self.lock:
            ...
            recent_failures = [f for f in self.failure_history[-10:] if not f[2]]  # Last 10 failures
            
            if not recent_failures:
                return self._calculate_base_delay(attempt)
            
            # Calculate average delay that failed
            avg_failed_delay = sum(f[1] for f in recent_failures) / len(recent_failures)
    ...
    def get_stats(self) -> Dict:
        ...
        with self.lock:
            ...
            successes = sum(1 for _, _, success in self.failure_history if success)
            ...
            recent_failures = [f for f in self.failure_history[-10:] if not f[2]]
            ...
            return {
                ...
                'avg_delay': sum(delay for _, delay, _ in self.failure_history) / total_attempts,
