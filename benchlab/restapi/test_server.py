        ...
        try:
            ...
            test_config = type('TestConfig', (), {
                ...
                'validate': lambda self: (
                    None if self.POLL_INTERVAL >= 0.1 else exec('raise ValueError("POLL_INTERVAL must be at least 0.1 seconds")'),
                    None if self.HISTORY_LENGTH >= 1 else exec('raise ValueError("HISTORY_LENGTH must be at least 1")'),
                    None if 1 <= self.API_PORT <= 65535 else exec('raise ValueError("API_PORT must be between 1 and 65535")'),
                    None if self.MAX_HISTORY_LIMIT >= 1 else exec('raise ValueError("MAX_HISTORY_LIMIT must be at least 1")'),
                    None if self.SCAN_INTERVAL >= 1 else exec('raise ValueError("SCAN_INTERVAL must be at least 1 second")')
                )[0]
            })()
