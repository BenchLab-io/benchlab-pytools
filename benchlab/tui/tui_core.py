...
class TUICore:
    ...
    def _handle_fleet_keys(self, key: str) -> Optional[Dict[str, Any]]:
        ...
        """
        if key == 'KEY_UP':
            # Let caller know fleet_index changed, they need to provide device count
    ...
    def _render_header(self, width: int):
        ...
        try:
            self.stdscr.addstr(0, 0, header.center(width), 
                             curses.A_BOLD | curses.color_pair(Config.COLOR_PAIRS['header']))
        except curses.error:
            pass
        ...
        for i, name in enumerate(Config.TAB_NAMES):
            ...
            try:
                if i == self.current_tab:
                    self.stdscr.addstr(2, tab_x, label, 
                                     curses.color_pair(Config.COLOR_PAIRS['header']) | curses.A_BOLD)
                else:
                    self.stdscr.addstr(2, tab_x, label, 
    ...
    def _render_current_tab(self, snapshot: Dict[str, Any], stats: ChannelStats,
                          fleet_devices: List[Dict[str, Any]], refresh_interval: float,
    ...
    def _render_status_bar(self, snapshot: Dict[str, Any], height: int, width: int):
        ...
        try:
            ...
            device_str = snapshot.get('uid') or snapshot.get('port') or "no device"
    ...
    def _render_fleet_tab(self, snapshot: Dict[str, Any], fleet_devices: List[Dict[str, Any]], 
                         height: int, width: int):
        """Render Fleet tab."""
        ...
        if not fleet_devices:
            try:
                self.stdscr.addstr(6, 4, "No connected devices found.", 
                                 curses.color_pair(Config.COLOR_PAIRS['error']))
            except curses.error:
                pass
            return
        ...
        hdr_attr = curses.A_UNDERLINE | curses.color_pair(Config.COLOR_PAIRS['default'])
        ...
        try:
            ...
            self.stdscr.addstr(6, cols['STATUS'], f"{'Status':<{widths['STATUS']}}", hdr_attr)
        ...
        for i, dev in enumerate(fleet_devices):
            ...
            row_color = (curses.color_pair(Config.COLOR_PAIRS['header']) | curses.A_BOLD 
                        if i == self.fleet_index else curses.color_pair(Config.COLOR_PAIRS['default']))
            ...
            try:
                ...
                if is_busy:
                    self.stdscr.addstr(row, cols['FW'], f"{'N/A':<{widths['FW']}}", row_color)
                    self.stdscr.addstr(row, cols['UID'], f"{dev_uid:<{widths['UID']}}", 
                                     curses.color_pair(Config.COLOR_PAIRS['caution']) | curses.A_BOLD)
                else:
                    try:
                        ...
                        fw_str = "0x????????"
                    self.stdscr.addstr(row, cols['FW'], f"{fw_str:<{widths['FW']}}", row_color)
                    self.stdscr.addstr(row, cols['UID'], f"{dev_uid:<{widths['UID']}}", row_color)
                
                self.stdscr.addstr(row, cols['STATUS'], f"{status:<{widths['STATUS']}}", status_color)
                
                if is_active:
                    self.stdscr.addstr(row, cols['ACTIVE'], "*", 
                                     curses.color_pair(Config.COLOR_PAIRS['ok']) | curses.A_BOLD)
            except curses.error:
                pass
        ...
        try:
            ...
            if connected and ct:
                ...
                self.stdscr.addstr(height - 2, 2, f"Uptime: {uptime}", 
                                 curses.color_pair(Config.COLOR_PAIRS['caution']))
            else:
                self.stdscr.addstr(height - 2, 2, "Uptime: —", 
                                 curses.color_pair(Config.COLOR_PAIRS['error']))
        except curses.error:
            pass

    def _render_device_tab(self, snapshot: Dict[str, Any], refresh_interval: float):
        """Render Device tab."""
    ...
    def _render_system_tab(self, snapshot: Dict[str, Any], stats: ChannelStats):
        """Render System tab."""
        ...
        for (key, label), val in zip(Config.Channels.POWER_SUMMARY, sum_vals):
            ...
            self._draw_bar(row, 4, label, val, 'W', max_sum,
    ...
    def _render_voltage_tab(self, snapshot: Dict[str, Any], stats: ChannelStats):
        """Render Voltage tab."""
        if not snapshot.get('connected') or not snapshot.get('sensor_data'):
        ...
        for key, label, band in Config.Channels.BOARD_VOLTAGES:
            val = sd.get(key, 0.0)
            stat = stats.get(uid, key)
            self._draw_bar(row, 4, label, val, 'V', Config.BarScales.BOARD_VOLTAGE_MAX,
                         curses.color_pair(Config.COLOR_PAIRS['voltage']), stat=stat, decimals=3)
            row += 1
        ...
        for vin_key in Config.Channels.VIN_CHANNELS:
            ...
            self._draw_bar(row, 4, vin_key, val, 'V', Config.BarScales.VIN_VOLTAGE_MAX,
                         curses.color_pair(Config.COLOR_PAIRS['voltage']), stat=stat, decimals=3)
            row += 1

    def _render_temperature_tab(self, snapshot: Dict[str, Any], stats: ChannelStats):
        ...
        if chip_temp is None:
            chip_temp = 0.0
        chip_color = Config.get_temperature_color(chip_temp, Config.BarScales.CHIP_TEMP_WARN)
        stat = stats.get(uid, 'Chip_Temp')
        self._draw_bar(row, 4, 'Chip Temp', chip_temp, '°C', Config.BarScales.CHIP_TEMP_MAX,
                     curses.color_pair(chip_color), stat=stat, decimals=1)
        row += 2
        ...
        for key, label in [('Ambient_Temp', 'Ambient Temp'), ('Humidity', 'Humidity')]:
            val = sd.get(key, 0.0)
            stat = stats.get(uid, key)
            ...
            self._draw_bar(row, 4, label, val, unit, max_val,
                         curses.color_pair(Config.COLOR_PAIRS['info']), stat=stat, decimals=1)
            row += 1
    ...
    def _render_fans_tab(self, snapshot: Dict[str, Any], stats: ChannelStats):
        ...
        for i in range(1, num_fans + 1):
            ...
            bar_len = max(0, min(20, int((rpm / Config.BarScales.FAN_RPM_MAX) * 20)))
    ...
    def _draw_disconnected(self, tab_name: str):
        ...
        try:
            self.stdscr.addstr(6, 4, f"{tab_name} telemetry unavailable — device disconnected.",
                             curses.color_pair(Config.COLOR_PAIRS['error']))
        except curses.error:
            pass
    ...
    def _draw_bar(self, y: int, x: int, label: str, value: float, unit: str, max_val: float, 
        ...
        try:
            self.stdscr.addstr(y, x, f"{label:<14} {val_str} {unit:<3} ", color)
            ...
            if stat and stat[0] is not None:
                ...
                self.stdscr.addstr(stat_str, curses.color_pair(Config.COLOR_PAIRS['default']))
        except curses.error:
            pass