...
class TUIApplication:
    ...
    def _handle_action(self, action: Dict[str, Any]) -> bool:
        ...
        if action_type == 'quit':
            ...
            self.tui_core.set_status(f"Fleet rescanned ({source_label}) — {count} device(s) found.")
    ...
    def _connect_datasource(self):
        ...
        try:
            if self.datasource_manager.connect():
                uid = self.datasource_manager.get_selected_uid()
                self.tui_core.set_status(f"Connected via {self.source_type.upper()}")
                logger.info(f"Connected to {self.source_type} datasource, selected: {uid}")
            else:
                snapshot = self.datasource_manager.snapshot()
                error_msg = snapshot.get('last_error', f"Failed to connect to {self.source_type}")
                self.tui_core.set_status(f"Connection failed: {error_msg}", 5.0)
                logger.error(f"Failed to connect to {self.source_type}: {error_msg}")
    ...
    def _connect_to_device(self, device: Dict[str, Any]):
        ...
        if uid == "BUSY":
            self.tui_core.set_status("Device is busy (already connected elsewhere)", 3.0)
            return
